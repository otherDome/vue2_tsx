 ...
 */
export function startDrag(bar:any, target:any, callback:any) {
  var params= {
    top: 0,
    left: 0,
    currentX: 0,
  ...
  bar.onmousedown = function (event:any) {
    ...
    document.onmousemove = function (event) {
      // 防止文字选中
      (window as any).getSelection ? (window as any).getSelection().removeAllRanges() : (document as any).selection.empty();

      var e:any = event ? event : window.event;
      if (params.flag) {
        var nowX = e.clientX;
        var nowY = e.clientY;
        // 差异距离
